...
# captured off the board while it played a clip generated by tools/sync_disc.py
# -- our own content, so no third-party frame ships in the repo. The build draws
# the HUD 14 px LEFT of the nominal X0, close enough to the 16 px cell pitch that
# a grid one cell over also fits every glyph exactly. This fixture locks that.
...
# block lattice is shifted by the SAME offset, which made the nominal-centre
...
def selftest():
    ...
    if os.path.exists(REAL_BAND):
        ...
        checks = [
            ...
            ('time', '0:00:04/0:04:00' in row['text'], True),
        ]
    ...
    if os.path.exists(REAL_BLOCKS):
        ...
        # Steady playback of a generated clip: video is live, but no menu
        # highlight is armed and no subpicture is on screen. The row-3 blocks
        # are gated off (Progressive), so they must read "not shown" rather
        # than False -- that distinction is the point of the fixture.
        want = {'_blocks_found': 8, 'hl_btns_armed': False, 'video_live': True,
                'subpic_shown': False, 'vbuf_deep': False, 'still_active': False,
