/* trace_wait.c -- WHERE DOES libdvdnav ASK THE PLAYER TO SYNC?  (2026-09-15)
 *
 * A fork of trace_nav.c that REPORTS DVDNAV_WAIT instead of swallowing it
 * (trace_nav does `case DVDNAV_WAIT: dvdnav_wait_skip(nav); break;`).
 *
 * libdvdnav raises it at dvdnav.c:869 when LEAVING a cell that is a still or
 * carries an active HLI:
 *
 ...
 *      before this, otherwise it might show stills or menus too shortly *\/
 *   if ((position_current.still || pci.hli.hl_gi.hli_ss) && !sync_wait_skip)
 *       sync_wait = 1;
 *
 * and the SOURCE STOPS until the app calls dvdnav_wait_skip() -- VLC drains its
 * decoder there, with a timeout (modules/access/dvdnav.c DVDNAV_WAIT).
 *
 * This fork exists to answer one question with the oracle rather than by
 * reasoning: does a real player get a sync point at the menu junctions that fry
 * on our core (docs/quant_matrix.md §13)?  Every WAIT is printed with the VM
 * state it was raised at, so it can be matched against the PGC the board shows.
 *
 * Original trace_nav.c header follows.
 *
...
int main(int argc, char **argv) {
  ...
  int wait_cells = 0, cells = 0, waits = 0;
  ...
  while (!finished) {
    ...
    case DVDNAV_STILL_FRAME: {
      ...
      break; }
    case DVDNAV_WAIT: {
      /* THE MEASUREMENT: libdvdnav wants the app in sync before this cell's
       * state changes land. Report where, then release it like any player. */
      dvd_state_t *st = &nav->vm->state;
      printf("[*** DVDNAV_WAIT ***] dom=%d vts=%d pgc=%d cell=%d   "
             "(libdvdnav asked the player to DRAIN here)\n",
             st->domain, st->vtsN, st->pgcN, st->cellN);
      waits++;
