...
| Closed captions (line-21 / CC) | ✅ analog line-21 re-insertion — **HW-CONFIRMED 2026-08-26** (C1 on a real TV, MiB/Matrix) | Extracted in `vld.v` from MPEG-2 user_data, re-modulated onto line 21 of the analog raster for the TV to decode (`dvd/cc_line21.sv`, `O[14]`). **No on-screen renderer** — nothing on HDMI; dropped on fit grounds (98% ALMs). Prevalence MEASURED: **6/34 local discs** carry live EIA-608 (all NTSC); `tools/cc_scan.py`, `dvd_census.py --captions`. Format + decode design: `docs/closed_captions.md` |
| Multi-angle | ✅ | Phase 9 |
