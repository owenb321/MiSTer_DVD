...
| Wiring | `dvd/emu.sv` | `spu_decode` fed by `core_h_pos/core_v_pos/av_stc` — since 2026-09-02 the query coordinates are **context-selected**: menu/highlight contexts go through the `crt_ov_map` Letterbox/Crop inverse, plain subtitles query raw raster space (`docs/crt_anamorphic.md` §9a); `subpic_blend` at the debug-overlay tap (overlay on top). One alignment register matches the 1-cyc read latency; `CE_PIXEL` delay bumped 2→3 to keep the CRT gated CE aligned. |
