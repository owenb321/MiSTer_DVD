...
⏳ HW-confirm pending. **Next:** flash `dev-tmapseek`, D-pad arm on the rig with
`tools/pause_match.py` against the v0.7.0 control numbers; the held scrub needs a gamepad by
hand (the harness cannot hold one).
...
**patent** notice and is independent of all of this.

> ⚠ **Corrected 2026-08-26.** This section used to claim "the AC-3 decoder is clean of
> liba52's GPL — liba52 is a co-sim *oracle*, never source material." **That was wrong**,
> and the repository's own comments contradict it: `dvd/ac3/ac3_tables.svh` says its
> values are "transcribed verbatim from liba52", `dvd/ac3/ac3_imdct_tables.svh` is
> generated by `bench/ac3/gen_imdct_tables.c` (which links liba52 and includes its
> private `a52_internal.h`), and three modules describe themselves as literal
> transcriptions of named liba52 functions. The decoder is an original RTL
> *architecture* but a **derivative work** of liba52's algorithms — which is fine, since
> GPL-2.0-or-later composes into GPL-3.0-or-later; it just needs attributing rather than
> disclaiming. See `NOTICE` for the full statement.
