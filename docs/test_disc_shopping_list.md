...
| 15 | **Closed captions (line-21)** | ~~likely out of scope~~ — **superseded 2026-08-25, and there is nothing to shop for**: the local library already covers it. `tools/cc_scan.py` measured **6/34 discs** with live EIA-608 (MiB, Matrix, Castle in the Sky, Clue, Elmopalooza, Paw Patrol) plus Roger Waters as a *carrier-only* disc (well-formed blocks, all-null payload) — a free negative-test case. Zero CEA-708, zero field-2/XDS anywhere, so those paths can't be shopped for either. It is still true that the TV normally decodes it; that fact is now the IMPLEMENTATION: the core re-inserts line 21 on the analog output for the TV to decode (`dvd/cc_line21.sv`, `docs/closed_captions.md`). Those six discs plus Roger Waters are the HW-gate test set. |
