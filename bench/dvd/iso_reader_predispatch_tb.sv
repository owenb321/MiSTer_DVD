// iso_reader_predispatch_tb.sv - REPRO for the Tomb Raider freeze
// (memory: single-button-in-title-hli-render / game-disc-freeze-hw-diagnosis).
//
// Tomb Raider dead-ends at VMGM PGC4, a *1-cell PRE-dispatcher*: a menu PGC
// whose real work is 7 PRE commands ending in LinkPGCN, but which carries ONE
// DEGENERATE cell (still=0, cmd_nr=0, ~0 s playback). The existing vm_tb T2
// proves a *0-cell* stub's PRE runs via the VM; this tb adds the 1-cell
// degenerate case, which the reader currently streams as linear content
// instead of running its PRE -> the LinkPGCN never fires -> S_DONE freeze.
//
// Reach (faithful to TR: VTS3 PGC3 POST = CallSS VMGM):
//   BOOT: FP {JumpTT 1} -> title VTS_01 PGC(0x81) streams 0xB0, POST =
//         CallSS VMGM (pgc 2).
//   Title ends -> drain -> vm_pgc_end -> POST CallSS VMGM pgc 2.
//   VMGM PGC2 = 1-cell DEGENERATE dispatcher, pre {g5=0xAB; LinkPGCN 3},
//              cell {0,0} streams 0xC1 (the null body).
//   VMGM PGC3 = a REAL menu, streams 0xC2, cell cmd = LinkCN 1 (loop).
//
// PASS = the reader follows PGC2's PRE LinkPGCN to PGC3: gprm[5]==0xAB AND
//        0xC2 streams (never dead-ends on 0xC1).
//
...
module iso_reader_predispatch_tb;
    ...
    // ---- event trace (post-boot; enable for debugging) ----
    reg trace_on = 0;
    always @(posedge clk) if (trace_on) begin
        if (vm_jump_pulse)
            $display("  [%0t] VM jump_pulse dom=%0d vts=%0d pgcn=%0d",
                     $time, vm_jump_domain, vm_jump_vts, vm_jump_pgcn);
        if (pgc_loaded)
            $display("  [%0t] pgc_loaded nr_pre=%0d cur_pgcn=%0d menu_active=%b",
                     $time, nr_pre_w, cur_pgcn_rd, menu_active);
    end
    ...
    task build_iso;
        ...
        begin
            ...
            put_rec(cur, 29, 6144, 8'h00, "VIDEO_TS.VOB;1", 14, cur); // VMGM menu VOB 29..31
            ...
            // VMGM PGCI_UT @21: 3 SRPs at pit_abs = 21*2048+16
            //   SRP[0] entry 0x82 pgc@64  = boot PGC (1 cell 0xC0, unused here)
            //   SRP[1] entry 0x00 pgc@300 = PGC2 = 1-cell DEGENERATE dispatcher
            //   SRP[2] entry 0x00 pgc@600 = PGC3 = REAL menu (0xC2, LinkCN 1 loop)
            pit = 21*2048 + 16;
            put_ut(21, 16'd3);
            put_srp(21, 0, 8'h82, 32'd64);
            put_srp(21, 1, 8'h00, 32'd300);
            put_srp(21, 2, 8'h00, 32'd600);
            // boot PGC
            put_pgc(pit+64, 8'd1, 8'd1, 16'd0, 8'd0, 16'd0, 16'd0, 16'd256);
            put_cell(pit+64, 16'd256, 0, 8'd0, 8'd0, 32'd0, 32'd0);   // 0xC0 @29
            // PGC2: 1-cell dispatcher, pre {g5=0xAB; LinkPGCN 3}, cell {0,0} 0xC1.
            // cmd table (236..259: 8 hdr + 2*8) must NOT overlap the cell (cpo>=260).
            put_pgc(pit+300, 8'd1, 8'd1, 16'd0, 8'd0, 16'd236, 16'd0, 16'd280);
            put_cmdtbl(pit+300, 16'd236, 2, 0, 0,
                       64'h7100000500AB0000,               // g[5] = 0xAB
                       64'h2004000000000003,               // LinkPGCN 3
                       64'd0);
            put_cell(pit+300, 16'd280, 0, 8'd0, 8'd0, 32'd1, 32'd1);  // 0xC1 @30
            // PGC3: real menu, cell cmd 1 = LinkCN 1 (replay cell 0 = loop)
            put_pgc(pit+600, 8'd1, 8'd1, 16'd0, 8'd0, 16'd320, 16'd0, 16'd256);
            put_cell(pit+600, 16'd256, 0, 8'd0, 8'd1, 32'd2, 32'd2);  // 0xC2 @31
            put_cmdtbl(pit+600, 16'd320, 0, 0, 1,
                       64'h2007000000000001,               // LinkCN 1
                       64'd0, 64'd0);
            ...
            // 1 pgm/cell {0,0}, POST = CallSS VMGM (pgc 2, rsm_cell 1)
            be16(23*2048+0, 16'd1);
            img[23*2048+8] = 8'h81;               // SRP[0].entry_id (title 1)
            be32(23*2048+8+4, 32'd16);
            put_pgc(23*2048+16, 8'd1, 8'd1, 16'd0, 8'd0, 16'd320, 16'd240, 16'd256);
            img[23*2048+16+240] = 8'd1;           // pm[0] = cell 1
            put_cell(23*2048+16, 16'd256, 0, 8'd0, 8'd0, 32'd0, 32'd0);  // 0xB0 @27
            put_cmdtbl(23*2048+16, 16'd320, 0, 1, 0,
                       64'h3008000201c00000,      // CallSS VMGM (pgc 2)
                       64'd0, 64'd0);

            // VTSM PGCI_UT @24: PGCN1 entry 0x83 @64 = 0-cell stub (unused)
            put_ut(24, 16'd1);
            put_srp(24, 0, 8'h83, 32'd64);
            put_pgc(24*2048+16+64, 8'd0, 8'd0, 16'd0, 8'd0, 16'd0, 16'd0, 16'd0);

            // payload sectors
            for (j = 0; j < 2048; j = j + 1) begin
                img[25*2048+j] = 8'hD0;           // menu VOB cell 1
                img[26*2048+j] = 8'hD1;           // menu VOB cell 2
                img[27*2048+j] = 8'hB0;           // title cell
                img[28*2048+j] = 8'hB1;           // title cell 2 (unused)
                img[29*2048+j] = 8'hC0;           // VMGM boot cell
                img[30*2048+j] = 8'hC1;           // VMGM PGC2 degenerate cell
                img[31*2048+j] = 8'hC2;           // VMGM PGC3 real menu cell
            end
        end
    endtask
    ...
    initial begin
        ...
        // ---------------- title end -> POST CallSS VMGM pgc2 -----------
        // Wait for the reader to load the VMGM dispatcher and (correctly)
        // follow its PRE LinkPGCN 3 to PGC3 (0xC2).  Poll up to a big budget.
        begin : wait_c2
            integer t; integer seen_c1; integer seen_c2;
            t = 0; seen_c1 = 0; seen_c2 = 0;
            while (t < 8000000 && seen_c2 == 0) begin
                @(posedge clk);
                if (stream_valid && stream_data == 8'hC1) seen_c1 = seen_c1 + 1;
                if (stream_valid && stream_data == 8'hC2) seen_c2 = seen_c2 + 1;
                t = t + 1;
            end
        ...
        if (errors == 0) $display("ISO_READER_PREDISPATCH_TB: ALL TESTS PASSED");
        else             $display("ISO_READER_PREDISPATCH_TB: FAILED with %0d errors", errors);
        $finish;
    end
    ...
    initial begin
        ...
        $display("  state=%0d vm_state=%0d cap_n=%0d menu=%b cur_vts=%0d cur_pgcn=%0d",
                 dut.state, vm.state, cap_n, menu_active, cur_vts, cur_pgcn_rd);
        $finish;
    end
