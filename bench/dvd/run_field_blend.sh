...
if [ $RED = 1 ]; then
  ...
  mutm() {
    ...
    launch "mut_$1_mod" "$1_mod" fail "$3" $(mod_args "$FX/fblend_synth")
  }
  ...
  mutc MC1 dvd/resample_addrgen.v 's/: blend_scan ? (oline >= frm_H)/: blend_scan ? (oline >= (frm_H - 12'"'"'d1))/' "C2"
  mutc MC2 dvd/resample_addrgen.v 's/disp_y <= (oline == (frm_H - 12.d1)) ? disp_y - 12.d1 : disp_y + 12.d1;/disp_y <= (oline == (frm_H - 12'"'"'d1)) ? disp_y : disp_y + 12'"'"'d1;/' "C1B"
  mutc MC3 dvd/resample_addrgen.v 's/blend_en \&\& deinterlace \&\& ~interlaced \&\& cur_ilace \&\&/blend_en \&\& deinterlace \&\& ~interlaced \&\&/' "C4" +pfr=1
  ...
  mutc MC7 dvd/field_blend.sv 's/  reg         r_wr;/  reg         r_wr;\n  reg         tgl;/; s/      blend_act <= 1.b0;/      blend_act <= 1'"'"'b0; tgl <= 1'"'"'b0;/; s/      if (ft_arr) blend_act <= sb_blend;/      if (ft_arr) begin blend_act <= sb_blend; tgl <= ~tgl; end/; s/r_y   <= p2_blend ? k_y\[9:2\]  : p2_d\[31:24\];/r_y   <= p2_blend ? (k_y[9:2] ^ {7'"'"'d0, tgl}) : p2_d[31:24];/' "C1 C1B C1T C5"
  score
  ...
  m=$(mutate W1 dvd/emu.sv 's/wire blend_en = status\[49\] \& ~interlaced_eff;/wire blend_en = ~status[49] \& ~interlaced_eff;/') && wred W1_default_on --emu "$m"
  m=$(mutate W2 dvd/emu.sv 's/wire blend_en = status\[49\] \& ~interlaced_eff;/wire blend_en = status[49] \& ~fields_eff;/') && wred W2_fields_gate --emu "$m"
  m=$(mutate W3 dvd/emu.sv 's/\.blend_en          (blend_en_dec)/.blend_en          (blend_en)/') && wred W3_no_cdc --emu "$m"
  m=$(mutate W4 dvd/emu.sv 's/"O\[49\],Progressive Deint,Off,Blend;"/"O[49],Progressive Deint,Blend,Off;"/') && wred W4_values_swapped --emu "$m"
