...
module av_sync_tb;
  ...
  reg rst_n = 0;
  reg  [33:0] mirror_data = 0; reg mirror_valid = 0;
  ...
  av_sync dut (.clk(clk), .rst_n(rst_n),
    .mirror_data(mirror_data), .mirror_valid(mirror_valid),
  ...
  task automatic send(input [32:0] v, input a);
    begin mirror_data <= {a, v}; mirror_valid <= 1; @(posedge clk); mirror_valid <= 0; @(posedge clk); end
  endtask
  ...
  initial begin
    ...
    for (i = 0; i < 5; i = i + 1) begin
      send(33'd100000 + i * 300, 1'b1); @(posedge clk);
    ...
    send(33'h1_FFFF_FF00, 1'b1);                              // near the top of the 33-bit range
