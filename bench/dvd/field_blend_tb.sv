...
module field_blend_tb;
  ...
  initial begin
    ...
    $display("==== field_blend_tb BLEND=%0d plain=%0d stall=%0d  %0dx%0d ft=%0d ====",
             BLEND, plain, stall, W, H, ft_code);
    ...
    if (BLEND || plain) begin
      ...
      if (act_seen !== (plain ? 1'b0 : 1'b1)) begin
        $display("FAIL: [K4] blend_act seen=%0d, want %0d", act_seen, plain ? 0 : 1); errors = errors + 1;
      end
    end else begin
      if (bad_px != 0)
