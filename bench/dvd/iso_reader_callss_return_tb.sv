// iso_reader_callss_return_tb.sv - coverage for the "choice -> clip -> CallSS
// VMGM -> return to the same choice" loop (Tomb Raider return-to-choice idiom).
// NOTE: this SIMPLIFIED model does NOT reproduce the real HW freeze (the RTL
// handles this loop correctly here) -- it guards that the CallSS-VMGM round-trip
// + in-title LinkPGCN keep the VTS and re-enter the choice PGC.
//
// HW finding (user): choices that ADVANCE work; choices that play a clip and
// RETURN to the same choice screen FREEZE. On the disc the return routes through
// a CallSS VMGM menu round-trip that JumpTTs back to the title:
//   VTS3 PGC..(choice) -> LinkPGCN (clip) -> ... -> CallSS VMGM (pgc N)
//     -> VMGM dispatcher -> JumpTT <title> -> back to the choice PGC.
// The forward path stays in the title; only the CallSS-VMGM-return loops back.
//
// This tb mirrors that loop minimally:
//   FP {JumpTT 1} -> VTS1 PGC1 (choice, 0xB0), POST LinkPGCN 2
//     -> VTS1 PGC2 (clip, 0xB1), POST CallSS VMGM (pgc 2)
//     -> VMGM PGC2 (dispatcher), pre JumpTT 1  -> BACK to VTS1 PGC1 (0xB0).
// PASS = the loop CLOSES: 0xB0 streams AGAIN after the CallSS-VMGM round-trip
// (i.e. the return-to-choice completes), cur_vts==1, reader not dead-ended.
//
// Run: iverilog -g2012 -o /tmp/rcr dvd/dvd_iso_reader.sv dvd/dvd_vm.sv \
//        dvd/bcd_time_add.sv bench/dvd/iso_reader_callss_return_tb.sv && vvp /tmp/rcr

`timescale 1ns/1ps

module iso_reader_callss_return_tb;
    ...
    // trace
    reg trace_on = 0;
    reg [5:0] prev_rs = 0; reg [3:0] prev_vs = 0; reg detail = 0;
    always @(posedge clk) if (trace_on) begin
        if (vm_jump_pulse)
            $display("  [%0t] jump dom=%0d vts=%0d pgcn=%0d", $time, vm_jump_domain, vm_jump_vts, vm_jump_pgcn);
        if (pgc_loaded)
            $display("  [%0t] pgc_loaded cur_vts=%0d cur_pgcn=%0d menu=%b nr_post=%0d", $time, cur_vts, cur_pgcn_rd, menu_active, nr_post_w);
        if (vm_pgc_end_w) $display("  [%0t] vm_pgc_end", $time);
        if (vm_cell_cmd_w) $display("  [%0t] vm_cell_cmd nr=%0d", $time, cur_cell_cmdnr_w);
        if (detail) begin
            if (dut.state != prev_rs) $display("  [%0t] rd_state %0d -> %0d", $time, prev_rs, dut.state);
            if (vm.state != prev_vs)  $display("  [%0t] vm_state %0d -> %0d", $time, prev_vs, vm.state);
            if (dut.state == 6'd32)   // S_PGC_CELLCHK
                $display("  [%0t] CELLCHK nr_cells=%0d cell_pb_off16=%0d pgc_sec=%0d pgc_off=%0d pit_sec=%0d srp_i=%0d nr_srp_l=%0d cmd_nr_pre=%0d",
                         $time, dut.nr_cells, dut.cell_pb_off16, dut.pgc_sec, dut.pgc_off, dut.pit_sec, dut.srp_i, dut.nr_srp_l, dut.cmd_nr_pre);
        end
        prev_rs <= dut.state; prev_vs <= vm.state;
    end
    ...
    endtask
    task put_ut(input integer sec, input [15:0] nsrp); integer base; begin
        base=sec*2048; be16(base+0,16'd1); be32(base+4,32'd1000); be16(base+8,16'h656E);
        img[base+10]=0; img[base+11]=8'h80; be32(base+12,32'd16); be16(base+16,nsrp); be32(base+20,32'd2000);
    end endtask
    task put_srp(input integer sec, input integer idx, input [7:0] entry_id, input [31:0] pgc_start);
        integer a; begin a=sec*2048+16+8+idx*8; img[a]=entry_id; be32(a+4,pgc_start); end
    endtask
    ...
    task build_iso;
        ...
        begin
            ...
            put_rec(cur,29,2048,8'h00,"VIDEO_TS.VOB;1",14,cur);
            put_rec(cur,22,6144,8'h00,"VTS_01_0.IFO;1",14,cur);   // 22..24
            put_rec(cur,27,6144,8'h00,"VTS_01_1.VOB;1",14,cur);   // title 27..29

            // VMGI_MAT @19: FP @400; TT_SRPT @+1 (20); VMGM_PGCI_UT @+2 (21)
            be32(19*2048+132,32'd400); be32(19*2048+196,32'd1); be32(19*2048+200,32'd2);
            put_pgc(19*2048+400,8'd0,8'd0,16'd0,8'd0,16'd236,16'd0,16'd0);
            put_cmdtbl(19*2048+400,16'd236,2,0,0,64'h7100000E00350000,64'h3002000000010000,64'd0); // g14=0x35; JumpTT 1

            // TT_SRPT @20: 1 title -> VTS_01
            be16(20*2048+0,16'd1); img[20*2048+14]=8'd1; img[20*2048+15]=8'd1;

            // VMGM PGCI_UT @21: SRP[0] entry 0x82 boot (0xC0); SRP[1] entry 0x00 = dispatcher
            pit=21*2048+16;
            put_ut(21,16'd2);
            put_srp(21,0,8'h82,32'd64);
            put_srp(21,1,8'h00,32'd320);
            put_pgc(pit+64,8'd1,8'd1,16'd0,8'd0,16'd0,16'd0,16'd256);
            put_cell(pit+64,16'd256,0,8'd0,8'd0,32'd0,32'd0);              // boot cell 0xC0 @29 (unused)
            // dispatcher PGC (entry-scanned as pgcn 2): 1 degenerate cell, pre = JumpTT 1
            // (cmd tbl 236..251 = 8 hdr + 1*8; cell @ 260 to avoid overlap)
            put_pgc(pit+320,8'd1,8'd1,16'd0,8'd0,16'd236,16'd0,16'd260);
            put_cmdtbl(pit+320,16'd236,1,0,0,64'h3002000000010000,64'd0,64'd0); // JumpTT 1
            put_cell(pit+320,16'd260,0,8'd0,8'd0,32'd1,32'd1);              // degenerate 0xC1 @30

            // VTSI_MAT @22: vts_pgcit=+1 (23)
            be32(22*2048+204,32'd1);
            // title VTS_PGCIT @23: 2 SRPs
            //   SRP[0] entry 0x81 -> PGC1 @32: 1 cell 0xB0, POST LinkPGCN 2
            //   SRP[1] entry 0x01 -> PGC2 @640: 1 cell 0xB1, POST CallSS VMGM (pgc 2)
            be16(23*2048+0,16'd2);
            img[23*2048+8]=8'h81;  be32(23*2048+8+4, 32'd32);    // SRP[0] @ off 8 -> PGC1
            img[23*2048+16]=8'h01; be32(23*2048+16+4, 32'd640);  // SRP[1] @ off 16 -> PGC2
            put_pgc(23*2048+32,8'd1,8'd1,16'd0,8'd0,16'd320,16'd240,16'd256);
            img[23*2048+32+240]=8'd1;
            put_cell(23*2048+32,16'd256,0,8'd0,8'd0,32'd0,32'd0);          // 0xB0 @27
            put_cmdtbl(23*2048+32,16'd320,0,1,0,64'h2004000000000002,64'd0,64'd0); // POST LinkPGCN 2
            put_pgc(23*2048+640,8'd1,8'd1,16'd0,8'd0,16'd320,16'd240,16'd256);
            img[23*2048+640+240]=8'd1;
            put_cell(23*2048+640,16'd256,0,8'd0,8'd0,32'd1,32'd1);         // 0xB1 @28
            // POST = CallSS VMGM (pgc 2, rsm_cell 1) = 30 08 00 02 01 c0 00 00
            put_cmdtbl(23*2048+640,16'd320,0,1,0,64'h3008000201c00000,64'd0,64'd0);

            for (j=0;j<2048;j=j+1) begin
                img[27*2048+j]=8'hB0; img[28*2048+j]=8'hB1; img[29*2048+j]=8'hC0; img[30*2048+j]=8'hC1;
            end
        end
    endtask
    ...
    initial begin
        ...
        t1_end=cap_n;
        $display("BOOT: VTS1 PGC1 0xB0 cur_vts=%0d (cap=%0d)", cur_vts, t1_end);

        // PGC1 POST LinkPGCN 2 -> PGC2 (0xB1)  [advance, my fix keeps vts]
        begin : wb1 integer t; integer s; t=0; s=0;
            while (t<8000000 && s==0) begin @(posedge clk); if (stream_valid && stream_data==8'hB1) s=1; t=t+1; end
            if (s==0) fail("advance: LinkPGCN 2 did not reach PGC2 (0xB1)");
        end
        t2_end=cap_n;
        $display("advance: PGC2 0xB1 reached cur_vts=%0d", cur_vts);
        detail=1;   // trace state transitions through the return

        // PGC2 POST CallSS VMGM (pgc 2) -> VMGM dispatcher -> JumpTT 1 -> BACK to
        // VTS1 PGC1 (0xB0 AGAIN) = the return-to-choice. THE FREEZE POINT.
        begin : wb2 integer t; integer s;
            t=0; s=0;
            while (t<12000000 && s==0) begin
                @(posedge clk);
                if (stream_valid && stream_data==8'hB0) s=s+1;   // 0xB0 again = looped back
                t=t+1;
            end
            $display("  after CallSS VMGM return: reader state=%0d vm_state=%0d cur_vts=%0d cur_pgcn=%0d menu=%b B0-again=%0d",
                     dut.state, vm.state, cur_vts, cur_pgcn_rd, menu_active, s);
            if (s==0)
                fail("FREEZE REPRO: CallSS VMGM -> JumpTT 1 return did NOT loop back to the choice (0xB0)");
            if (cur_vts!==8'd1)
                fail("return landed on the wrong VTS (cur_vts != 1)");
        end

        if (errors==0) $display("ISO_READER_CALLSS_RETURN_TB: ALL TESTS PASSED");
        else           $display("ISO_READER_CALLSS_RETURN_TB: FAILED with %0d errors", errors);
        $finish;
    end

    initial begin
        #700000000;
        $display("GLOBAL TIMEOUT state=%0d vm_state=%0d cap_n=%0d cur_vts=%0d cur_pgcn=%0d menu=%b",
                 dut.state, vm.state, cap_n, cur_vts, cur_pgcn_rd, menu_active);
        $finish;
    end
