...
module dpad_seek_tb;
    ...
    initial begin
        ...
        // ---- T16: request saturates at UNIT_CAP (240 s) -------------------
        arm;
        press(2'd2); press(2'd2); press(2'd2); press(2'd2); press(2'd2);  // 5x60 = 300 s
        if (pend_tens !== 8'd24) begin
            $display("FAIL T16 pend_tens=%0d want 24 (capped)", pend_tens);
            errors = errors + 1; end
        expect_fire("T16 saturate at 240 s = 2 x fwda[0]", 32'd60000, 1'b1, LBN0);
