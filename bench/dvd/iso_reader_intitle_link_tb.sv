// iso_reader_intitle_link_tb.sv - regression for the in-title LinkPGCN VTS fix
// (dvd_vm.sv link_jump_vts; PR #145). Guards that an in-title cross-PGC LinkPGCN
// keeps the title's VTS and loads the target PGC (not the best-VTS fallback).
//
// TR is a rare MULTI-PGC game title: the game navigates PGC->PGC inside one VTS
// via cross-PGC LinkPGCN (returning to an earlier choice screen = a LinkPGCN
// back to an earlier PGC). The title is entered via JumpTT, which sets the VM's
// vm_vts=0 (the VTS is resolved in the reader, not the VM). A subsequent
// in-title LinkPGCN shipped jump_vts <= vm_vts = 0, so the reader tried to load
// "VTS 0" instead of staying in the current title -> mis-resolve -> dead-end
// (S_DONE), video freeze. Single-PGC movie titles never do a cross-PGC LinkPGCN,
// so they never hit it. (The identical bug was fixed for JumpVTS_TT long ago,
// dvd_vm.sv "Scene It boot bug", by using cur_vts; LinkPGCN was missed.)
//
// This tb: FP {JumpTT 1} -> VTS_01 PGC1 (0xB0), POST = LinkPGCN 2 -> the reader
// must STAY in VTS_01 and stream PGC2 (0xB1). PASS = 0xB1 streams and cur_vts==1
// (before the fix: cur_vts drops to 0, PGC2 is not reached, reader dead-ends).
//
// Run: iverilog -g2012 -o /tmp/ril dvd/dvd_iso_reader.sv dvd/dvd_vm.sv \
//        dvd/bcd_time_add.sv bench/dvd/iso_reader_intitle_link_tb.sv && vvp /tmp/ril

`timescale 1ns/1ps

module iso_reader_intitle_link_tb;
    ...
    task put_cell(input integer pa, input [15:0] cpo, input integer idx,
        ...
        begin
            c = pa + cpo + idx*24;
            img[c+2] = still; img[c+3] = cmdnr; be32(c+8, first); be32(c+20, last);
        end
    endtask
    ...
    task put_cmdtbl(input integer pa, input [15:0] off,
        ...
        begin
            a = pa + off;
            be16(a+0, npre[15:0]); be16(a+2, npost[15:0]); be16(a+4, ncell[15:0]); be16(a+6, 16'd0);
            n = npre + npost + ncell;
    ...
    task build_iso;
        ...
        begin
            ...
            img[32768] = 8'd1;
            img[32769]="C"; img[32770]="D"; img[32771]="0"; img[32772]="0"; img[32773]="1"; img[32774]=8'd1;
            ...
            put_rec(cur, 27, 4096, 8'h00, "VTS_01_1.VOB;1", 14, cur); // title VOB

            // VMGI_MAT @19: FP @400; TT_SRPT @+1 (20)
            be32(19*2048+132, 32'd400);
            be32(19*2048+196, 32'd1);
            // FP PGC: pre = {g14=0x35, JumpTT 1}
            put_pgc(19*2048+400, 8'd0, 8'd0, 16'd0, 8'd0, 16'd236, 16'd0, 16'd0);
            put_cmdtbl(19*2048+400, 16'd236, 2, 0, 0,
                       64'h7100000E00350000, 64'h3002000000010000, 64'd0);

            // TT_SRPT @20: 1 title -> VTS_01, vts_ttn 1
            be16(20*2048+0, 16'd1);
            img[20*2048+14] = 8'd1;   // title_set_nr = 1 (VTS 1)
            img[20*2048+15] = 8'd1;   // vts_ttn = 1

            // VTSI_MAT @22: vts_pgcit=+1 (23)
            be32(22*2048+204, 32'd1);

            // title VTS_PGCIT @23: 2 SRPs (MULTI-PGC title):
            //   SRP[0] entry 0x81 -> PGC1 @16: 1 cell 0xB0, POST = LinkPGCN 2
            //   SRP[1] entry 0x01 -> PGC2 @600: 1 cell 0xB1, POST = LinkPGCN 1 (loop)
            // Title VTS_PGCIT SRP table starts at sector offset 8 (no LU indirection):
            //   SRP[i] @ offset 8 + i*8, pgc_start @ +4.
            be16(23*2048+0, 16'd2);
            img[23*2048+8]  = 8'h81; be32(23*2048+8+4,  32'd32);   // SRP[0] @off 8 -> PGC1@32
            img[23*2048+16] = 8'h01; be32(23*2048+16+4, 32'd640);  // SRP[1] @off 16 -> PGC2@640
            put_pgc(23*2048+32, 8'd1, 8'd1, 16'd0, 8'd0, 16'd320, 16'd240, 16'd256);
            img[23*2048+32+240] = 8'd1;                                  // pm[0]=cell1
            put_cell(23*2048+32, 16'd256, 0, 8'd0, 8'd0, 32'd0, 32'd0);  // 0xB0 @27
            put_cmdtbl(23*2048+32, 16'd320, 0, 1, 0,
                       64'h2004000000000002, 64'd0, 64'd0);              // POST LinkPGCN 2
            ...
            put_cmdtbl(23*2048+640, 16'd320, 0, 1, 0,
                       64'h2004000000000001, 64'd0, 64'd0);              // POST LinkPGCN 1 (loop)

            for (j = 0; j < 2048; j = j + 1) begin
                img[27*2048+j] = 8'hB0;   // title PGC1 cell (first=0 -> sector 27)
                img[28*2048+j] = 8'hB1;   // title PGC2 cell (first=1 -> sector 28)
                img[29*2048+j] = 8'hC0;
            end
        end
    endtask
    ...
    begin
        t = 0;
        while (cap_n < target && t < 6000000) begin @(posedge clk); t = t + 1; end
    ...
    initial begin
        ...
        // BOOT: FP -> JumpTT 1 -> VTS_01 PGC1 (0xB0)
        wait_bytes(2048);
        if (cap[0] !== 8'hB0) fail("BOOT: first byte != B0 (VTS1 PGC1 not reached)");
        if (cur_vts !== 8'd1) fail("BOOT: cur_vts != 1 after JumpTT 1");
        t1_end = cap_n;
        $display("BOOT: FP -> JumpTT 1 -> VTS1 PGC1 0xB0, cur_vts=%0d  (cap=%0d)", cur_vts, t1_end);

        // PGC1 POST = LinkPGCN 2 (in-title cross-PGC link). The reader MUST stay
        // in VTS_01 and stream PGC2 (0xB1). Before the fix, LinkPGCN ships
        // jump_vts=vm_vts=0 -> reader mis-resolves "VTS 0" -> dead-end, no 0xB1.
        begin : wait_b1
            integer t; integer seen_b1;
            t = 0; seen_b1 = 0;
            while (t < 8000000 && seen_b1 == 0) begin
                @(posedge clk);
                if (stream_valid && stream_data == 8'hB1) seen_b1 = seen_b1 + 1;
                t = t + 1;
            end
            $display("  after PGC1 POST LinkPGCN 2: reader state=%0d cur_vts=%0d cur_pgcn=%0d menu_active=%b 0xB1=%0d",
                     dut.state, cur_vts, cur_pgcn_rd, menu_active, seen_b1);
            if (seen_b1 == 0)
                fail("FREEZE REPRO: in-title LinkPGCN 2 did not reach VTS1 PGC2 (0xB1 never streamed)");
            if (cur_vts !== 8'd1)
                fail("in-title LinkPGCN dropped the VTS (cur_vts != 1)");
            if (cur_pgcn_rd !== 8'd2)
                fail("in-title LinkPGCN did not load PGC2 (cur_pgcn != 2 = linear fallback)");
        end

        if (errors == 0) $display("ISO_READER_INTITLE_LINK_TB: ALL TESTS PASSED");
        else             $display("ISO_READER_INTITLE_LINK_TB: FAILED with %0d errors", errors);
        $finish;
    end
