...
module cc_line21 (
    ...
    // ★ CAPTION CREDIT (2026-09-07): one pulse per DISPLAY pickup carrying that
    // picture's field count. See the drain note below -- this is what puts the
    // caption stream on the display's clock instead of the raster's.
    input             dec_credit_valid,
    input      [2:0]  dec_credit,
...
// ---------------------------------------------------------------------------
// ---------------------------------------------------------------------------
// ★ THE DRAIN RUNS ON THE DISPLAY, NOT THE RASTER (2026-09-07).
//
// The pairs for a whole GOP arrive as ONE BURST when the VLD parses the GOP
// header, then leave two per frame. Draining them on raster fields matches the
// RATE -- the raster and the display run at the same rate by construction -- but
// fixes nothing about the PHASE, which is simply whatever the queue's occupancy
// happened to be, i.e. about a GOP (~0.5 s), constant, and never re-aligned.
//
// A credit arrives with every display PICKUP carrying that picture's field count,
// and EIA-608 is one pair per field with the 3:2 expansion already done by the
// encoder (cc_count counts DISPLAY frames). So "release this many pairs" and
// "put this picture on screen" are the same event, and the GOP's captions drain
// exactly as the GOP's pictures are displayed.
//
// ⚠ FAILURE MODE IS DELIBERATE. If credits stop arriving (a mode where pickups do
// not reach here, or content with captions but a stalled display), CREDIT_WD frees
// the drain after ~1 s of a non-empty queue, so the worst case degrades to the old
// free-running behaviour rather than to silence. A decorative side channel must
// never be able to wedge, and it must never hold the FIFO full enough to refuse
// writes for long.
// ⚠ Credits SATURATE. Un-captioned content grants credits nothing consumes, and an
// unbounded counter would bank minutes of them and make the gate inert forever
// afterwards.
// ---------------------------------------------------------------------------
localparam [5:0]  CREDIT_MAX = 6'd32;
localparam [24:0] CREDIT_WD  = 25'd27_000_000;   // ~1 s at 27 MHz

// ⚠ INITIALISED, not left to a reset. This toggle lives in the dec_clk domain,
// which has no reset here, and an unreset toggle is X forever in simulation
// (~X is X) and undefined at power-up in silicon -- the credit would simply never
// arrive and every pop would block. Same declaration-initialiser idiom the rest of
// the fork uses for such counters (cf. flush_ctl's aud_flush_cnt).
reg  cr_tog_d = 1'b0;                      // toggle from the dec_clk side
reg  [2:0] cr_amt_d = 3'd0;
always @(posedge dec_clk) begin
    if (dec_credit_valid) begin
        cr_tog_d <= ~cr_tog_d;
        cr_amt_d <= dec_credit;
    end
end
reg  cr_t1 = 1'b0, cr_t2 = 1'b0, cr_t3 = 1'b0;
always @(posedge clk) begin cr_t1 <= cr_tog_d; cr_t2 <= cr_t1; cr_t3 <= cr_t2; end
wire cr_pulse = cr_t3 ^ cr_t2;

reg  [5:0]  credits;
reg  [24:0] cr_wd;
wire        cr_free = (cr_wd == CREDIT_WD);    // watchdog: drain anyway
...
always @(posedge clk) begin
    if (!rst_n) begin
        ...
        // credits in; saturate so an un-captioned stretch cannot bank them
        if (cr_pulse && (credits <= (CREDIT_MAX - 6'd6)))
            credits <= credits + {3'd0, cr_amt_d};
        else if (cr_pulse && (credits < CREDIT_MAX))
            credits <= CREDIT_MAX;

        // watchdog: a non-empty queue that is not draining for ~1 s frees itself
        if (fifo_empty || fifo_rd) cr_wd <= 25'd0;
        else if (cr_wd != CREDIT_WD) cr_wd <= cr_wd + 25'd1;

        if (flush) begin
            slot_v   <= 2'd0;
            hold_v   <= 1'b0;
            draining <= 1'b1;
            credits  <= 6'd0;      // the new stream re-earns them from its own pickups
            cr_wd    <= 25'd0;
        end else begin
            ...
            if (want_pop) begin
                fifo_rd <= 1'b1;
                rd_busy <= 1'b1;
                // spend one credit per pair. `draining` (a flush's backlog dump)
                // and the watchdog both pop without one -- neither is a scheduled
                // release, so neither should owe anything.
                if (!draining && (credits != 6'd0)) credits <= credits - 6'd1;
            end
            if (fifo_valid) begin
                rd_busy <= 1'b0;
                if (!draining) begin
