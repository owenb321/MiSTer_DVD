module mem_shim (
    ...
    // =========================================================================
    // 2-State FSM + Skid Buffer + read-response serialization
    // =========================================================================
    // state=0 (IDLE): Accept new FIFO commands
    // state=1 (WAIT): Wait for DDR3 bridge acceptance (!waitrequest)
    //
    // *** DDR3 deadlock fix (dvd/ copy — rtl/ original left untouched) ***
    // The original FSM only waited for a read to be *accepted* (!waitrequest),
    // then immediately issued the next command — frequently a write — while the
    // read's `readdatavalid` was still outstanding. On-hardware overlay probing
    // (rows 0-2: rd_count 62 > rsp_count 61, shim_state stuck at D = WRITE/WAIT,
    // stall counter saturated) showed this wedges the HPS f2sdram port: it
    // withholds the read response and pins `waitrequest` high forever, so the
    // trailing write never completes and the decoder stalls (black screen, all
    // three MPEG2/DVD cores). The proven MiSTer DDR3 users (sys/ddr_svc.sv, the
    // ascal scaler) never do this — they fully drain a read's response before
    // issuing the next command. So we serialize the same way: `read_outstanding`
    // tracks the single in-flight read (burstcnt is always 1 => one beat per
    // read) and S_IDLE refuses to issue ANY new command until its
    // `readdatavalid` has returned.
    //
    ...
    // Single outstanding read tracker (serialization fix — see header above).
    // Set when a read command is accepted by the bridge; cleared when its
    // readdatavalid returns. While set, S_IDLE issues no new command.
    reg        read_outstanding;

    // Read-response timeout / give-up (survive a dropped read).
    // On hardware the f2sdram bridge intermittently accepts a read then never
    // returns readdatavalid (rd_count leads rsp_count by one; the stuck read
    // waits the full 65535-cycle probe window). The serializer then correctly
    // refuses to wedge — but the decoder starves and watchdogs out. Rather than
    // block forever, time out a non-responding read after RD_TIMEOUT cycles
    // (>> the ~tens-of-cycles normal latency) and synthesize a response so the
    // decoder keeps running. If drops are occasional the decoder limps on
    // (glitchy video); if systematic it stays black and the drop count climbs.
    // Strategy: when a read times out, RE-ISSUE it (the serializer has held off
    // all writes meanwhile, so the f2sdram write FIFO has drained — a read that
    // was dropped due to colliding with the write backlog should now succeed and
    // return the CORRECT data). Only after RD_MAX_RETRY failed re-issues do we
    // give up and synthesize a zero so the decoder can't be starved forever.
    localparam [11:0] RD_TIMEOUT   = 12'd2047;  // cycles before a read is presumed lost
    localparam [2:0]  RD_MAX_RETRY = 3'd6;      // re-issues before giving up
    reg [11:0] rd_wait_cnt;
    reg [2:0]  rd_retry_cnt;
    wire read_wait_expired = read_outstanding && (rd_wait_cnt == RD_TIMEOUT);
    wire read_retry        = read_wait_expired && (rd_retry_cnt <  RD_MAX_RETRY);
    wire read_giveup       = read_wait_expired && (rd_retry_cnt == RD_MAX_RETRY);
    ...
    always @(posedge clk) begin
        if (!rst_n) begin
            ...
            // -----------------------------------------------------------------
            // A real readdatavalid forwards the DDR3 data. Only after the retries
            // are exhausted do we synthesize a zero so the decoder isn't starved.
            if (read_giveup) begin
                mem_res_wr_en  <= 1'b1;
            ...
            // Read-response wait counter: runs while a read is outstanding and
            // unanswered; reset on response, on expiry (retry/give-up), or idle.
            if (read_outstanding && !ddr3_readdatavalid && !read_wait_expired)
                rd_wait_cnt <= rd_wait_cnt + 1'd1;
            else
                rd_wait_cnt <= 0;

            // Retry counter: counts re-issues of the current read; cleared when
            // no read is in flight (so each fresh read gets a full retry budget).
            if (!read_outstanding)
                rd_retry_cnt <= 0;
            else if (read_retry)
                rd_retry_cnt <= rd_retry_cnt + 1'd1;
            ...
            if (!state) begin
                ...
                // =============================================================
                if (read_retry) begin
                    // Read timed out with retries left: re-issue the SAME read.
                    // ram_address still holds its address (untouched while blocked)
                    // and the write FIFO has drained during the wait, so this
                    // re-issue should return the correct data.
                    ram_read      <= 1'b1;
                    state         <= 1'b1;   // -> S_WAIT
                    mem_req_rd_en <= 1'b0;
                end
                else if (read_outstanding) begin
                    // Serialize: a read's response is still in flight. Issue no
                    // new command (read or write) until readdatavalid returns —
                    // issuing a write here is exactly what wedges the f2sdram
                    // bridge. Hold by not pulling new FIFO data; any command at
                    // the FIFO output or in the skid buffer stays put.
                    mem_req_rd_en <= 1'b0;
                end
            ...
            // -----------------------------------------------------------------
            // Outstanding-read tracker (serialization fix)
            // -----------------------------------------------------------------
            // Set when our read command is accepted by the bridge; cleared when
            // its single readdatavalid beat returns. The S_IDLE gate keeps at
            // most one read in flight, so set and clear never collide.
            if (ram_read && !ddr3_waitrequest)
                read_outstanding <= 1'b1;
            else if (ddr3_readdatavalid || read_giveup)
