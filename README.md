...
- **liba52 0.8.0** — more than a reference: the in-fabric AC-3 decoder follows its
  algorithms function-by-function, its bit-allocation ROM values are transcribed from it,
  and the IMDCT tables are generated by a program that links it. The RTL architecture is
  this project's own, but `dvd/ac3/` is properly described as a **derivative work** of
  liba52 (GPL-2.0-or-later, compatible with this project's GPL-3.0-or-later — see
  [NOTICE](NOTICE)). It is *also* the co-simulation golden: every coding tool is checked
  against it to a bounded error, and it caught bugs (a mis-decoded dynamic-range exponent
  among them) that listening tests did not.
